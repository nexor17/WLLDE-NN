...
class PricePredictor(nn.Module):
    # Advanced LSTM with MultiheadAttention and Residual Connections
    def __init__(self, input_size, dropout=0.3, hidden_size=512):
        ...
        # Multi-head attention with more heads for better pattern recognition
        self.attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=16, 
        ...
        # Deeper fully connected layers with residual connections
    ...
    def forward(self, x):
        ...
        lstm_out1, _ = self.lstm1(x)  # (batch, seq_len, hidden_size*2)
        
        # Second LSTM layer
        lstm_out2, _ = self.lstm2(lstm_out1)  # (batch, seq_len, hidden_size)
        
        # Layer normalization
        lstm_out2 = self.layer_norm1(lstm_out2)
        
        # Multi-head attention with residual connection
        attn_out, _ = self.attention(lstm_out2, lstm_out2, lstm_out2)
        attn_out = self.layer_norm2(attn_out + lstm_out2)  # Residual connection
        
        # Global average pooling + last timestep (ensemble approach)
        avg_pool = torch.mean(attn_out, dim=1)  # (batch, hidden_size)
        last_step = attn_out[:, -1, :]  # (batch, hidden_size)
        x = (avg_pool + last_step) / 2  # Combine both
        
        # Deep FC layers with residual-like connections
        x = self.fc1(x)
